...
export class PostsService {
  ...
  async update(id: number, updatePostDto: UpdatePostDto, currentUserId: number): Promise<PostResponseDto> {
    const post = await this.prisma.post.findUnique({
      where: { id },
    });
  ...
  async getUserPosts(userId: number, currentUserId?: number): Promise<PostResponseDto[]> {
    const posts = await this.prisma.post.findMany({
      where: {
        authorId: userId,
      },
